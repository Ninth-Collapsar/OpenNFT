    ...
    case 'DICOM'
        ...
        if useTCPData && (indVol > 1)
            ...
            while isempty(vol) || contains(lastwarn,'Suspicious fragmentary file')
                ...
                fprintf(['******* ', num2str(indVol), 'th file size = ', num2str(fileinfo.bytes)]);
                [filepath, name, extname] = fileparts(fileName);
                bufferFilename = fullfile(filepath,'..','buffer', 'temp.dcm');
                %fprintf(['******', bufferFilename]);
                copyfile(fileName, bufferFilename);
                vol = double(dicomread(bufferFilename));
            end
